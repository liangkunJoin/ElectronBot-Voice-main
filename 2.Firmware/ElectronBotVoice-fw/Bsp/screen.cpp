...
void Screen::ChipSelect(bool _enable) {
    _enable ? LCD_CS_GPIO_Port->BSRR = (uint32_t) LCD_CS_Pin << 16U : LCD_CS_GPIO_Port->BSRR = LCD_CS_Pin;
}
...
void Screen::SetDataOrCommand(bool _isData) {
    _isData ? LCD_DC_GPIO_Port->BSRR = LCD_DC_Pin : LCD_DC_GPIO_Port->BSRR = (uint32_t) LCD_DC_Pin << 16U;
}
